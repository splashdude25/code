...
chosen_word = wordList[r]
# or
# chosen_word = random.choice(wordList)
#set up your blanks to be filled in
guess_word = []
...
while play:
    #show game so far
    print(drawings[strikes])
    print(guess_word)
    print(f'You have guessed {letters_guessed} so far')
    #Have player guess and verify it is a single letter
    invalidGuess = True
    letter = ''
    while invalidGuess:
    ...
    if strikes >= maxStrikes:
        ...
        invalidPlayAgain = True
        while invalidPlayAgain:
            playAgain = input('Do you want to play again? (Y or N)')
            playAgain = playAgain.lower()
            if playAgain == 'y':
                play = True
                invalidPlayAgain = False
                r = random.randint(0, len(wordList)-1)
                chosen_word = wordList[r]
                # or
                # chosen_word = random.choice(wordList)
